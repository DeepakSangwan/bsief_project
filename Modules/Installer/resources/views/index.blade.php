...
@section('content')
    <div class="card">
            ...
            <div class="mb-3 row">
                <div class="col-12">
                    @if (!strtolower(config('app.app_mode')))
                        ...
                        <div class="p-1">
                            <p>
                                You are using demo mode. No access code needed. Continue installation.
                            </p>
                        </div>
                        <a href="{{ route('setup.requirements') }}" class="btn btn-success">Continue</a>
                    @else
                        <form id="verify_form">
                            <label for="purchase_code">Access Code</label>
        ...
        <div class="card-footer text-center">
            <p>For support, contact me at <a href="https://api.whatsapp.com/send/?phone=917988478587&text&type=phone_number&app_absent=0"
                target="_blank" rel="noopener noreferrer">@deepaksangwan</a>. I am here to help. Thank you!</p>
        </div>
    </div>
@endsection
@push('scripts')
    <script>
        $(document).ready(function() {
            $(document).on('submit', '#verify_form', async function(e) {
                ...
                if ($.trim(code) === '') {
                    toastr.warning("Access Code is required");
                } else {
                    submit_btn.html(
